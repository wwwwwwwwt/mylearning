 ...
 * @LastEditTime: 2023-03-04 17:21:47
...
int main() {
    ...
    timer.add(2, 8000, &bar); // 在2秒后执行bar()
    timer.add(3, 7000, &baz); // 在3秒后执行baz()
    timer.add(4, 1000, &foo); // 在1秒后执行foo()
    timer.add(5, 4000, &bar); // 在2秒后执行bar()
    timer.add(6, 3000, &baz); // 在3秒后执行baz()
